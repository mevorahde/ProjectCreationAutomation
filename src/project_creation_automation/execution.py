"""Confirmed local-only creation orchestration."""
...
class LocalCreationOrchestrator:
    ...
    def execute(self, request: ProjectRequest, plan: CreationPlan) -> ExecutionResult:
        ...
        try:
            ...
            return self._result(ExecutionStatus.SUCCEEDED, journal)
        except FilesystemMutationError as error:
            created = error.created
            return self._failed(request, journal, created, error)
        except DomainError as error:
            return self._failed(request, journal, created, error)

    @staticmethod
    def _reject_unavailable_actions(request: ProjectRequest) -> None:
        if request.create_github_repository or request.ide is not IDEChoice.NONE:
            raise UnsupportedExecutionError("non_local_execution_unavailable")
...
class ConsoleConfirmation:
    ...
    def confirm(self, plan: CreationPlan) -> bool:
        del plan
        self.output_stream.write("Proceed with local creation? [y/N]: ")
